...
const API_URL = String(runtimeConfig.apiBaseUrl || '').trim().replace(/\/$/, '') || '/api';
...
function imprimirOrcamento(orderId) {
  ...
  const logoSrc = branding.logoDataUrl || assetUrl('/images/Logo.png');
    ...
    carregarImpressoras().then(() => {
      app.innerHTML = `
        <div class="erp-layout">
          <aside class="sidebar">
            <div class="brand">
              <img src="${assetUrl('/images/Logo.png')}" alt="Infinity Maker Logo" class="brand-logo" />
            </div>
            ${renderSidebar()}
          </aside>
  ...
  if (configuracoesForm) {
    ...
    if (removeLogoButton) {
      removeLogoButton.addEventListener('click', () => {
        if (logoPreview) logoPreview.src = assetUrl('/images/Logo.png');
